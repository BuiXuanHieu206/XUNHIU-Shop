...
namespace SV20T1020375.Web.Controllers
{
    public class OrderController : Controller
    {
        ...
        [HttpPost]
        public IActionResult Details(int orderID = 0, string deliveryAddress = "", string deliveryProvince = "")
        {
            ...
            bool result = OrderDataService.ChangeAddressAndProvince(orderID, deliveryAddress, deliveryProvince);
            if (!result)
        ...
        /// <summary>
        /// Cập nhật giá bán và số lượng bán của một mặt hàng được bán trong đơn hàng
        /// Hàm trả về chuỗi khác rỗng thông báo lỗi nếu đầu vào không hợp lệ hoặc lỗi,
        /// hàm trả về chuỗi rỗng nếu thành công
        /// </summary>
        ...
        public IActionResult AddToCart(OrderDetail data)
        {
            ...
            else //Nết mặt hàng đã có trong giỏ thì tăng số lượng và thay đổi giá bán
            {
        ...
        {
            var shoppingCart = GetShoppingCart();
            var index = shoppingCart.FindIndex(m => m.ProductID == id);
            if (index >= 0)
            {
